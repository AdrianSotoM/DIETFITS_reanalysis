...
data_3m <- data %>% filter(redcap_event_name == "3_months_arm_1") %>% 
  ...
  mutate(delta_weight_3m = (data_bl$weight_gcrc - weight_gcrc)/data_bl$weight_gcrc) %>% 
...
data_6m <- data %>% filter(redcap_event_name == "6_months_arm_1") %>% 
  select(baseline_ins,gluc,diet,scr_gender,dxa_percentfat,
         ...
         saturated_fat..,fiber.g,carb.g,fat.g,protein.g,
         GI_glucose,GL_glucose,added_sugars,sugar.cal,thirty_ins) %>% 
  mutate(delta_weight_6m = (data_bl$weight_gcrc - weight_gcrc)/data_bl$weight_gcrc) %>% 
...
data_12m <- data %>% filter(redcap_event_name == "12_months_arm_1") %>% 
  select(baseline_ins,gluc,diet,scr_gender,dxa_percentfat,
         ...
         saturated_fat..,fiber.g,carb.g,fat.g,protein.g,
         GI_glucose,GL_glucose,added_sugars,sugar.cal,thirty_ins) %>% 
  mutate(delta_weight_12m = (data_bl$weight_gcrc - weight_gcrc)/data_bl$weight_gcrc) %>% 
...
##### WEIGHT LOSS AND LIPID MEASURES TIMEPOINT COMPARISONS #####
##### Weightloss #####
...
##### Panel B. Weight loss vs insulin at 30 min vs GL reduction #####
em <- lm(delta_weight_12m~bl30q5*deltaGLq5+diet+bl_bmi+bl_cal,
         data=data_12m)
summary(em)
em_betas <- ggcoefstats(em)
em_betas
...
######### PLEASE NOTE THAT MOST OBJECTS WILL BE OVER WRITTEN ###############
############################################################################
##### DATA FILTERING BY DIFFERENT TIMEPOINTS####
data_3m <- data %>% filter(redcap_event_name == "3_months_arm_1") %>% 
  select(baseline_ins,gluc,diet,scr_gender,dxa_percentfat,
...
##### Panel A. Carbointake reduction vs weight loss in the LCD (Def2) #####
lcd_adherence_12 <- ggscatterstats(
  data  = data_lcd_12m,
  x     = z_delta_carbcals_12m,
  y     = delta_weight_12m,
